...
import { createPWAAdapter } from '../utils/pwa-adapter'
...
export function useWebSocket() {
  ...
  const connected = ref(false) // PWA 连接状态
  ...
  const connect = () => {
    ...
    $http.get('server').then(response => {
      ...
      return new Promise((resolve, reject) => {
        ...
        ws.onopen = () => {
          resolve(ws)
          connected.value = true // PWA: 设置连接状态
        }
        ws.onerror = reject
      })
    }).then((ws) => {
      ...
      ws.onclose = () => {
        ...
        websocketConnecting.value = false
        connected.value = false // PWA: 设置断开状态
  ...
  const initPWA = () => {
    try {
      // 获取全局状态（需要从外部传入或使用provide/inject）
      const globalState = {
        websocket: websocket.value,
        websocketConnecting: websocketConnecting.value,
      ...
      console.log('[PWA] WebSocket PWA 适配器已初始化')
    } catch (error) {
      console.warn('[PWA] 适配器初始化失败:', error)
    }
  }
